...
abstract class AbstractControl<T> {
  ...
  set valuePerf(T? value) {
    updateValuePerf(value);
  }
  ...
  void updateValueAndValidity({
    ...
    if (_perf) {
      _updateValueAndValidityPerf(
        updateParent: updateParent,
        emitEvent: emitEvent,
      );
    } else {
      _updateValueAndValidity(updateParent: updateParent, emitEvent: emitEvent);
    }
  }
  ...
  /// Recalculates the value and validation status of this control and all
  /// its descendants using a depth-first search (DFS) traversal.
  ///
  /// This method first recursively updates all children, collecting their
  /// status and value information, then recalculates the status and value
  /// of this control based on the consolidated children results.
  ///
  /// Returns a record containing:
  /// - [status]: The consolidated [ControlStatus] of this control
  /// - [value]: The consolidated value of this control
  ///
  /// When [emitEvent] is true or not supplied (the default), both the
  /// *statusChanges* and *valueChanges* emit events with the latest status
  /// and value when the control is updated. When false, no events are emitted.
  ({ControlStatus status, Object? value}) _updateValueAndValidityPerf({
    bool updateParent = true,
    bool emitEvent = true,
  }) {
    print('_updateValueAndValidityPerf');
    if (updateParent) {
      AbstractControl<dynamic> root = this;
      while (root.parent != null) {
        root = root.parent!;
      }

      // Call updateValueAndValidity2 on the root to perform DFS traversal
      return root._updateValueAndValidityPerf(
        updateParent: false,
        emitEvent: emitEvent,
      );
    } else {
      List<ControlStatus>? listStatuses = <ControlStatus>[];

      forEachChild((child) {
        final childResult = child._updateValueAndValidityPerf(
          updateParent: updateParent,
          emitEvent: emitEvent,
        );
        listStatuses.add(childResult.status);
      });

      final mergedStatus = _mergeStatus(_status, listStatuses);

      _status = mergedStatus == ControlStatus.disabled
      ...
      if (enabled) {
        _cancelExistingSubscription();
        _errors = _runValidators();
        _status = _mergeStatus(_status, listStatuses);
        if (_status == ControlStatus.valid ||
            _status == ControlStatus.pending) {
          _runAsyncValidators();
        }
      }
      ...
      return (status: _status, value: _value);
    }
  }

  ControlStatus _mergeStatus(
    ControlStatus currentStatus,
    List<ControlStatus> other,
  ) {
    if (other.isEmpty && currentStatus == ControlStatus.disabled ||
        other.isNotEmpty && other.every((e) => e == ControlStatus.disabled)) {
      return ControlStatus.disabled;
    } else if (hasErrors) {
      return ControlStatus.invalid;
    } else if (other.isEmpty && currentStatus == ControlStatus.pending ||
        other.any((e) => e == ControlStatus.pending)) {
      return ControlStatus.pending;
    } else if (other.isEmpty && currentStatus == ControlStatus.invalid ||
        other.any((e) => e == ControlStatus.invalid)) {
      return ControlStatus.invalid;
    }
...
class FormGroup extends FormControlCollection<Map<String, Object?>> {
  ...
  void addAllLazy(Map<String, AbstractControl<dynamic>> controls) {
    _controls.addAll(controls);
    controls.forEach((name, control) {
      control.parent = this;
    });
